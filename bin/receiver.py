    ...
    while(1):
        if(state == "wait for 0 from below"):
            while(1):
                ...
                # check if packet is not corrupted and acked
                if(checksum_verifier(recv_message) and recv_message[0] == '0'):
                    ...
                    break
                # if packet is corrupted or incorrect SEQ number
                elif(not checksum_verifier(recv_message) or recv_message[0] == '1'):
        ...
        elif(state == "wait for 1 from below"):
            while(1):
                # received packet
                recv_message = clientSocket.recv(30).decode("utf-8")
                total_packet_recv += 1
                ...
                # check if packet is not corrupted and acked
                if(checksum_verifier(recv_message) and recv_message[0] == '1'):
                    state = "wait for 0 from below"
                    data += recv_message[4:24]
                    ...
                    break
                # if packet is corrupted or incorrect SEQ number
                elif(not checksum_verifier(recv_message) or recv_message[0] == '0'):
                    total_corrupted_pkt_recv += 1
